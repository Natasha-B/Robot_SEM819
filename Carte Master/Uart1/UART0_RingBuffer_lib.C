...
void ecoute (void){
	...
	if(stock[0]=='E'){																	// Fin Epreuve
		}*/
	/*if(stock[1]=='V')&(stock[0]='T'){								// TV vitesse
		TVvitesse = '';
		for (i=3;i<=strlen(stock);i++){
			str.append(TVvitesse,stock[i]);
			}
		}*/
	if(stock[0]=='A'){																	// Avancer	
		if (strlen(stock)==1){
			 strcpy(envoie,"mogo 1:20 2:20\r");
			}
		}
		/*else{
			vitesse = '';
			for (i = 2;i<strlen(stock);i++){
					str.append(vitesse,stock[i]);
				}
		...
		strcpy(envoie, "stop\r");
		}
	/*
	if(stock[1]=='D')&(stock[0]='R'){								// Rotation Droite 90°
		envoie = "digo 1:346:20 2:346:-20\r";
		}
	if(stock[1]=='G')&(stock[0]='R'){								// Rotation Gauche 90°
		envoie = "digo 1:346:-20 2:346:20\r"
		}	
	if(stock[1]=='C')&(stock[0]='R'){								// Rotation Complete 180°
		if (stock[3]=='D'){
	...
	if(stock[1]=='A')&(stock[0]='R'){						// Rotation angle donné
		if (stock[3]=='D'){
			if (strlen(stock)>5){
				AngleRotation='';
