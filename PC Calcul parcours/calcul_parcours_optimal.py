...
for j in range(size_terrain_dict):
    coord_terrain_x.append(int(coord_terrain[j].get("x")*echelle)) #coordonnées en x des sommets du terrain
    coord_terrain_y.append(int(coord_terrain[j].get("y")*echelle)) #coordonnées en y des sommets du terrain
...
min_x = int(min(coord_terrain_x))
max_x = int(max(coord_terrain_x))
min_y = int(min(coord_terrain_y))
max_y = int(max(coord_terrain_y))
...
def object_on_map(obj,terrain,value=1,min_x_terrain=0,min_y_terrain=0,marge=0,echelle=1):
    ...
    if obstacle_i_dict.get('type') == "cercle": #si l'obstacle est de forme circulaire
        ...
        centre_coord_x = ((list_sommet_ob_i[0].get('centre')).get("x"))*echelle
        centre_coord_y = ((list_sommet_ob_i[0].get('centre')).get("y"))*echelle
        r = (list_sommet_ob_i[1].get('rayon')+marge)*echelle     #rayon du cercle 
        """Avec autre echelle """
        ...
        for j in range(size_obj_dict):
            coord_obj_x.append((coord_obj[j].get("x"))*echelle) #coordonnées en x des sommets du terrain
            coord_obj_y.append((coord_obj[j].get("y"))*echelle) #coordonnées en y des sommets du terrain
        ...
        """Création de la modélisation du terrain en grille"""    
        """condition type"""
        min_x = int(min(coord_obj_x) - min_x_terrain-marge)
        max_x = int(max(coord_obj_x) - min_x_terrain+marge)
        min_y = int(min(coord_obj_y) - min_y_terrain-marge)
        max_y = int(max(coord_obj_y) - min_y_terrain+marge) 
...
for i in range (n_obstacles):
    ...
    object_on_map(obstacle_i_dict,terrain_numpy2,value,min_x,min_y,marge,echelle)
...
depart_x0 = int(((depart.get("coordonnees")).get("x"))*echelle)
depart_y0 = int(((depart.get("coordonnees")).get("y"))*echelle)

arrivee = data_dict_base_roul.get("arrivee")
arrivee_x0 = int(((arrivee.get("coordonnees")).get("x"))*echelle)
arrivee_y0 = int(((arrivee.get("coordonnees")).get("y"))*echelle)

tir = (data_dict_base_roul.get("position-tir_cible")).get("coordonnees")
tir_x0 = int((tir.get("x"))*echelle)
tir_y0 = int((tir.get("y"))*echelle)
...
if n_balises >0:
    for i in range(n_balises):  
        balise_data_dict 
        balise_i_x =int(((balise_data_dict [i].get("coordonnees")).get("x"))*echelle)
        balise_i_y =int(((balise_data_dict [i].get("coordonnees")).get("y"))*echelle)
        list_x_balises.append(ajustement_coord(min_x, balise_i_x))
        list_y_balises.append(ajustement_coord(min_y, balise_i_y))
    
for i in range(nb_etapes):  
    etape_i_x =int((((data_dict_base_roul.get("etapes"))[i].get("coordonnees")).get("x"))*echelle)
    etape_i_y =int((((data_dict_base_roul.get("etapes"))[i].get("coordonnees")).get("y"))*echelle)
...
for i in range(nb_etapes + n_balises + 2):
    if n_balises ==0 :
        if i == 0 : #entre départ et 1ere étape
            ...
            list_y_move.append(tir_y)
            way = astar(terrain_numpy2, (list_y_etapes[i-1],list_x_etapes[i-1]), (tir_y+1,tir_x+1),taille_r)
            for i in range(len(way)):
                list_x_move.append(way[i][1])
                list_y_move.append(way[i][0])
            plt.figure(1)
            ...
            list_x_move.append(list_x_etapes[i-2]) ##
            list_y_move.append(list_y_etapes[i-2]) 
            list_x_move.append(list_x_etapes[i-2]) 
            list_y_move.append(list_y_etapes[i-2]) 
            way = astar(terrain_numpy2, (arrivee_y,arrivee_x), (list_y_etapes[i-2]+1,list_x_etapes[i-2]+1),taille_r)
            for i in range(len(way)):
                list_x_move.append(way[i][1])
                list_y_move.append(way[i][0])
            plt.figure(1)
            ...
            list_y_move.append(list_y_etapes[i-2])
            list_y_move.append(list_y_etapes[i-2])
            way = astar(terrain_numpy2, (list_y_etapes[i-1],list_x_etapes[i-1]), (list_y_etapes[i-2],list_x_etapes[i-2]),taille_r)
            for i in range(len(way)):
                list_x_move.append(way[i][1])
                list_y_move.append(way[i][0])
            plt.figure(1)
        ...
        #way = astar(terrain_numpy2, (list_y_etapes[i],list_x_etapes[i]), (depart_y,depart_x),taille_r)
            ...
            list_y_move.append(tir_y)
            list_y_move.append(tir_y)
            way = astar(terrain_numpy2, (list_y_balises[i-1],list_x_balises[i-1]), (tir_y+1,tir_x+1),taille_r)
            for i in range(len(way)):
                list_x_move.append(way[i][1])
                list_y_move.append(way[i][0])
            plt.figure(1)
            ...
            list_x_move.append(list_x_balises[i-2])
            list_x_move.append(list_x_balises[i-2])
            list_y_move.append(list_y_balises[i-2])
            list_y_move.append(list_y_balises[i-2])
            way = astar(terrain_numpy2, (list_y_balises[i-1],list_x_balises[i-1]), (list_y_balises[i-2],list_x_balises[i-2]),taille_r)
            for i in range(len(way)):
                list_x_move.append(way[i][1])
                list_y_move.append(way[i][0])
            plt.figure(1)
            ...
            list_y_move.append(list_y_balises[i-2])
            list_y_move.append(list_y_balises[i-2])
            way = astar(terrain_numpy2, (list_y_etapes[i-3],list_x_etapes[i-3]), (list_y_balises[i-3],list_x_balises[i-3]),taille_r)
            for i in range(len(way)):
                list_x_move.append(way[i][1])
                list_y_move.append(way[i][0])
            plt.figure(1)
            ...
            list_x_move.append(list_x_etapes[i-4]) ##
            list_y_move.append(list_y_etapes[i-4]) 
            list_x_move.append(list_x_etapes[i-4]) 
            list_y_move.append(list_y_etapes[i-4]) 
            way = astar(terrain_numpy2, (arrivee_y,arrivee_x), (list_y_etapes[i-4]+1,list_x_etapes[i-4]+1),taille_r)
            for i in range(len(way)):
                list_x_move.append(way[i][1])
                list_y_move.append(way[i][0])
            plt.figure(1)
            ...
            list_y_move.append(list_y_etapes[i-4])
            list_y_move.append(list_y_etapes[i-4])
            way = astar(terrain_numpy2, (list_y_etapes[i-3],list_x_etapes[i-3]), (list_y_etapes[i-4],list_x_etapes[i-4]),taille_r)
            for i in range(len(way)):
                list_x_move.append(way[i][1])
                list_y_move.append(way[i][0])
            plt.figure(1)
...
while i < len(list_command)-1:
    ...
    if command == command_next:
        ...
        if data_dict_envir.get("unite") == "mm":
            avance = compt_avance * echelle /100
        elif data_dict_envir.get("unite") == "cm":
            avance = compt_avance * echelle /10
        elif data_dict_envir.get("unite") == "dm":
            avance = compt_avance * echelle
...
f =    open("C:/Users\David/parcours_optimal.txt","w")
...
f.close() 
dbx = dropbox.Dropbox('GA2yOgwmKSAAAAAAAAABL4BMab5h5PGP96vRhWCyO2hXKYBa7G1yEOFLzfPpHymV')
