...
for j in range(size_terrain_dict):
    coord_terrain_x.append(coord_terrain[j].get("x")) #coordonnées en x des sommets du terrain
    coord_terrain_y.append(coord_terrain[j].get("y")) #coordonnées en y des sommets du terrain
...
def object_on_map(obj,terrain,value=1,min_x_terrain=0,min_y_terrain=0, taille_robot=45,marge=0):
    ...
    if obstacle_i_dict.get('type') == "cercle": #si l'obstacle est de forme circulaire
        ...
        min_x = centre_coord_x -r - min_x_terrain 
        max_x = centre_coord_x + r - min_x_terrain 
        min_y = centre_coord_y - r - min_y_terrain 
        max_y = centre_coord_y + r - min_y_terrain 
        ...
        """Création de la modélisation du terrain en grille"""    
        """condition type"""
        min_x = min(coord_obj_x) - min_x_terrain - taille_robot
        max_x = max(coord_obj_x) - min_x_terrain + taille_robot
        min_y = min(coord_obj_y) - min_y_terrain - taille_robot
        max_y = max(coord_obj_y) - min_y_terrain + taille_robot
...
for i in range (n_obstacles):
    ...
    object_on_map(obstacle_i_dict,terrain_numpy2,value,min_x,min_y,taille_robot,marge)
...
#calcul du parcourspour chaque trajet:    
for i in range(nb_etapes + 2):
    if i == 0 : #entre départ et 1ere étape
        way = astar(terrain_numpy2, (list_y_etapes[i],list_x_etapes[i]), (depart_y,depart_x))
...
#        print(i)
        ...
        list_x_move.append(list_x_etapes[i-1])
        list_y_move.append(list_y_etapes[i-1])
        way = astar(terrain_numpy2, (tir_y,tir_x), (list_y_etapes[i-1]+1,list_x_etapes[i-1]+1))
        for i in range(len(way)):
            list_x_move.append(way[i][1])
            list_y_move.append(way[i][0])
        plt.figure(1)
        ...
        list_y_move.append(tir_y)
        way = astar(terrain_numpy2, (arrivee_y,arrivee_x), (tir_y+1,tir_x+1))
        for i in range(len(way)):
            list_x_move.append(way[i][1])
            list_y_move.append(way[i][0])
            plt.figure(1)
        ...
        list_y_move.append(list_y_etapes[i-1])
        list_y_move.append(list_y_etapes[i-1])
        way = astar(terrain_numpy2, (list_y_etapes[i],list_x_etapes[i]), (list_y_etapes[i-1]+1,list_x_etapes[i-1]+1))
        for i in range(len(way)):
            list_x_move.append(way[i][1])
            list_y_move.append(way[i][0])
            plt.figure(1)
...
list_command.append("Fin")  #permet d'avoir la derbière commande de déplacement                   
i = 0                    
compt_avance = 1
list_command2 = []
while i < len(list_command)-1:
    ...
    if command == command_next:
        ...
        compt_avance +=1
        if data_dict_envir.get("unite") == "cm":
        ...
        if command == "haut":
            ...
            if command_next == "haut":
                ...
                list_command2.append("S")
                i+=1
        elif command == "haut droite": ################################je suis ici
            if command_next == "haut":
                order = "G X:0 Y:"+ str(avance)+" A:45"
                list_command2.append(order)
                i += 1
